<template>
	<div class="userList" :class="{tableHeight: isExist,tabHeight: !isExist}" v-loading="loading">
		<el-table :data="tableData" style="width: 100%" @expand-change="changeExpand" stripe row-class-name="iconStyle" border ref="userList" :height="userTableHeight">
			<el-table-column type="selection" width="50"></el-table-column>
			<el-table-column type="expand">
				<template slot-scope="props">
					<user-list v-if="props.row.LOGIN" :id="Number(props.row.LOGIN)"></user-list>
				</template>
			</el-table-column>
...
import { getSubordinateUser , getTradeList , getTradeHistory } from "@/api/user/user";
...
export default {
	...
	data() {
		return {
			...
			tablePageSize: 10,
			tableHead: [
				{ propName: 'LOGIN',labelName: this.$t('table.username'),width: '80' },
				{ propName: 'NAME',labelName: this.$t('table.name'),width: '100' },
				{ propName: 'NAME',labelName: this.$t('table.role'),width: '100' },
				{ propName: 'NAME',labelName: this.$t('table.superior'),width: '100' },
				...
				{ propName: 'LASTDATE',labelName: this.$t('table.lasttime'),width: '150' }
			],
			loading: false,
			isExist: false,
			dialogVisible: false,
