<template>
	<div class="retailList" :class="{tableHeight: isExist,tabHeight: !isExist}" v-loading="loading">
		<el-table :data="tableData" style="width: 100%" @expand-change="changeExpand" stripe row-class-name="iconStyle" border ref="retailList" :height="userTableHeight">
			<el-table-column type="expand">
				<template slot-scope="props">
					<retail-list v-if="props.row.LOGIN" :id="Number(props.row.LOGIN)"></retail-list>
				</template>
			</el-table-column>
...
//import { getretailList } from "@/api/report/report";
	...
	data() {
		return {
			...
			tableHead: [
				...
				{ propName: 'REGDATE',labelName: this.$t('table.opendate'),width: '150' },
				{ propName: 'IN_DEPOSIT',labelName: this.$t('table.deposit'),width: '100' },
				{ propName: 'OUT_DEPOSIT',labelName: this.$t('table.withdraw'),width: '100' },
				{ propName: 'NET_DEPOSIT',labelName: this.$t('table.netdeposit'),width: '100' },
				{ propName: 'VOLUME',labelName: this.$t('indexLang.volume'),width: '100' },
				{ propName: 'BALANCE',labelName: this.$t('table.balance'),width: '100' },
				{ propName: 'EQUITY',labelName: this.$t('table.equity'),width: '100' },
				{ propName: 'COMMISSION',labelName: this.$t('table.commission'),width: '100' }
			],
			loading: false,
			isExist: false,
			dialogVisible: false,
...
<style>
.retailList .el-table th{
	background: rgb(223, 240, 216);
}
.retailList .el-table__expanded-cell[class*=cell]{
	padding: 10px 20px;
}
</style>
